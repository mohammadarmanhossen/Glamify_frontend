...
const displayProducts = (products) => {
  ...
  if (!Array.isArray(products) || products.length === 0) {
    ...
    productCart.innerHTML = products
      .map(
        (product) => `
    <div class="bg-white w-full rounded-2xl shadow-md hover:shadow-xl transition-shadow duration-300 overflow-hidden">
        <div class="w-full h-56 overflow-hidden">
            <img class="w-full h-full object-cover" src="${product.image_url}" alt="${product.name}" loading="lazy">
        </div>

        <div class="px-6 py-4">
            <h5 class="text-xl font-semibold text-gray-800 mb-1">${product.name}</h5>
            <p class="text-gray-500 text-sm mb-3 line-clamp-2">${product.description}</p>

            <div class="flex justify-between items-center mb-3">
                <span class="text-md font-bold text-green-400">${product.brand_name}</span>
                <span class="text-md font-bold text-green-400">৳${product.price}</span>
            </div>
