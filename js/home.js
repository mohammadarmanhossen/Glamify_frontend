    ...
    if (!Array.isArray(products) || products.length === 0) {
      ...
      productCart.innerHTML = products
        ...
        <div class="relative">
          ...
          <div class="px-6 py-4">
            <p class="text-xl font-semibold mb-1">${getShortName(product.name)}</p>
            <p class="text-gray-600 text-sm mb-3">${getShortDescription(product.description)}</p>
            <p class="text-gray-500 mb-4">Brand: ${product.brand_name}</p>
            <div class="flex justify-between items-center mt-4">
              <p class="font-semibold">${product.price}৳</p>
              <a href="${user_id ? `./product_details.html?id=${product.id}` : './login.html'}" class="text-sm font-semibold text-green-500  transition">
                View Details
              </a>
            </div>
          </div>
  ...
  const loadBrands = () => {
    ...
    fetch("https://glamify-backend-code.onrender.com/Brand/")
      ...
      .then(data => {
        ...
        data.forEach(brand => {
          if (brandList) brandList.appendChild(createBrandButton(brand.name, brand.id));
  ...
  const setupBrandFilter = () => {
    ...
    brandSelect.addEventListener("change", () => {
      ...
      document.querySelectorAll(".brand-btn").forEach(btn => {
        btn.classList.toggle("selected", btn.getAttribute("data-id") === selectedBrand);
      });
